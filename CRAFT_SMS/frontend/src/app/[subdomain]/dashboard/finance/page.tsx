  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-[var(--edlink-blue-text)] mb-2">
            Financial <span className="text-[var(--edlink-green-brand)]">Management</span>
          </h1>
          <p className="text-[var(--edlink-blue-text)]/70">
        ...
        {!isBusiness && (
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--edlink-green-brand)] hover:bg-[var(--edlink-green-hover)] text-white font-bold transition-all shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: 'Total Verified', value: slips.filter(s => s.status === 'VERIFIED').length, icon: CheckCircle2, bg: 'bg-[var(--edlink-green-brand)]/10', iconColor: 'text-[var(--edlink-green-brand)]', border: 'border-emerald-100' },
          ...
          <div key={i} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
            ...
            <p className="text-xs text-[var(--edlink-blue-text)]/70 uppercase tracking-widest font-bold mb-1">{stat.label}</p>
            <h3 className="text-2xl font-bold text-[var(--edlink-blue-text)]">{isLoading ? '—' : stat.value}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-4 p-2 bg-white border border-slate-200 rounded-2xl shadow-sm">
          ...
          {['ALL', 'PENDING', 'VERIFIED', 'REJECTED'].map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                filter === f
                  ? 'bg-[var(--edlink-green-brand)] text-white'
                  : 'text-[var(--edlink-blue-text)]/70 hover:text-[var(--edlink-blue-text)] hover:bg-slate-50'
              }`}
            >
              {f}
            </button>
        ...
        <div className="relative hidden md:block">
          ...
          <input
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[var(--edlink-green-brand)] pl-9 pr-4 py-2 text-sm w-64 text-[var(--edlink-blue-text)]"
          />
        </div>
      </div>
        ...
        {modalState !== 'NONE' && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {modalState === 'SUBMIT' && (
              ...
              >
                <button onClick={() => setModalState('NONE')} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-[var(--edlink-blue-text)]"><X className="w-5 h-5" /></button>
                <h2 className="text-2xl font-bold text-[var(--edlink-blue-text)] mb-2">Submit Slip</h2>
                <p className="text-sm text-[var(--edlink-blue-text)]/70 mb-6">Enter your payment details and upload a photo of your slip.</p>

                <form onSubmit={handleSubmitSlip} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-[var(--edlink-blue-text)] uppercase tracking-widest">Slip Number</label>
                      <input
                        required
                        value={newSlip.slip_number}
                        onChange={e => setNewSlip({ ...newSlip, slip_number: e.target.value })}
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[var(--edlink-green-brand)] text-[var(--edlink-blue-text)]"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-[var(--edlink-blue-text)] uppercase tracking-widest">Amount ($)</label>
                      <input
                        required
                        type="number"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-[var(--edlink-blue-text)] uppercase tracking-widest">Proof of Payment</label>
                    <label className="w-full h-32 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:border-[var(--edlink-green-brand)] hover:bg-[var(--edlink-green-brand)]/10 transition-all">
                      {newSlip.file ? (
                        <span className="text-sm text-[var(--edlink-green-brand)] font-bold">{newSlip.file.name}</span>
                      ) : (
                        <>
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-[var(--edlink-green-brand)] hover:bg-[var(--edlink-green-hover)] text-white font-extrabold rounded-2xl transition-all disabled:opacity-50"
                  >
            ...
            {modalState === 'IMAGE' && selectedSlip && (
              ...
              >
                <button onClick={() => setModalState('NONE')} className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 rounded-full text-[var(--edlink-blue-text)] z-20"><X className="w-6 h-6" /></button>
                {!selectedSlipUrl ? (
                  <div className="flex flex-col items-center justify-center space-y-4 text-slate-400">
                    <div className="w-8 h-8 border-4 border-[var(--edlink-green-brand)] border-t-transparent rounded-full animate-spin" />
                  ...
                  <div className="flex flex-col items-center justify-center space-y-4 text-rose-500">
                    ...
                    <p className="text-sm font-bold text-[var(--edlink-blue-text)]">Access Denied or File Not Found</p>
                  </div>
                ) : (
            ...
            {modalState === 'REJECT' && selectedSlip && (
              <motion.div
                ...
                className="bg-white rounded-2xl shadow-xl max-w-md w-full relative z-10 p-6"
              >
                <h3 className="text-xl font-bold text-[var(--edlink-blue-text)] mb-2">Reject Slip</h3>
                <p className="text-sm text-[var(--edlink-blue-text)]/70 mb-6">Reason for rejecting slip #{selectedSlip.slip_number}:</p>
                <textarea
                  value={rejectReason}
                  onChange={(e) => setRejectReason(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm focus:outline-none focus:border-[var(--edlink-green-brand)] text-[var(--edlink-blue-text)] min-h-[100px] mb-6"
                />
                <div className="flex gap-3">
                  <button onClick={() => setModalState('NONE')} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-[var(--edlink-blue-text)] rounded-xl font-bold transition-colors">Cancel</button>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white rounded-xl border border-slate-100">
            ...
            <p className="text-[var(--edlink-blue-text)]/70 font-medium">No slips found for this filter.</p>
          </div>
        ) : (
          slips.map((slip) => (
            <div key={slip.id} className="bg-white rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 p-5 hover:border-slate-200 hover:shadow-md transition-all">
              <div className="flex items-center gap-5">
                <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center font-bold text-sm ${
                  slip.status === 'VERIFIED' ? 'bg-[var(--edlink-green-brand)]/10 border-emerald-100 text-[var(--edlink-green-brand)]' :
                ...
                <div>
                  <h4 className="font-bold text-lg text-[var(--edlink-blue-text)]">{slip.profiles?.full_name ?? 'Unknown'}</h4>
              ...
              <div className="flex items-center gap-8">
                <div className="text-right">
                  <p className="text-xl font-bold text-[var(--edlink-blue-text)]">${slip.amount?.toFixed(2)}</p>
                  <span className={`text-[10px] font-black uppercase tracking-widest ${
                    slip.status === 'VERIFIED' ? 'text-[var(--edlink-green-brand)]' :
                ...
                <div className="flex items-center gap-2 border-l border-slate-100 pl-8">
                  <button
                    ...
                    className="p-3 rounded-xl bg-slate-50 text-[var(--edlink-blue-text)]/70 hover:text-[var(--edlink-blue-text)] hover:bg-slate-100 transition-all"
                  >
                  ...
                  {isBusiness && slip.status === 'PENDING' && (
                    <>
                      ...
                      <button onClick={() => handleVerify(slip.id, 'VERIFIED')} className="p-3 rounded-xl bg-[var(--edlink-green-brand)]/10 text-[var(--edlink-green-brand)] hover:bg-emerald-100 transition-all">
                  ...
                  {slip.status === 'VERIFIED' && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--edlink-green-brand)] hover:bg-[var(--edlink-green-hover)] text-white text-xs font-bold transition-all"
                    >
