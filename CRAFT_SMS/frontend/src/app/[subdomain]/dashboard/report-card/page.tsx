    ...
    <DashboardLayout>
      <div className="space-y-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
             <button onClick={() => router.back()} className="p-2 hover:bg-white/5 rounded-xl transition-all">
                <ChevronLeft className="w-6 h-6 text-[var(--edlink-blue-text)]/70" />
             </button>
             <div>
                <h1 className="text-3xl font-bold mb-1 text-white">Academic <span className="gradient-text">Report Card</span></h1>
                <p className="text-[var(--edlink-blue-text)]/70">Official institutional record for the selected term.</p>
             </div>
          </div>
          <div className="flex gap-3">
             ...
             <button 
               ...
               className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--edlink-green-brand)] text-black font-extrabold hover:bg-[var(--edlink-green-hover)] transition-all shadow-lg shadow-teal-500/20 disabled:opacity-50"
             >
        ...
        {isLoading ? (
           ...
           <div className="premium-card py-32 text-center text-[var(--edlink-blue-text)]/70">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="space-y-6">
                 <div className="premium-card bg-gradient-to-br from-teal-500/10 to-transparent">
                    <Trophy className="w-10 h-10 text-[var(--edlink-green-brand)] mb-4" />
                    ...
                    <p className="text-xs text-[var(--edlink-green-brand)] font-bold uppercase tracking-widest">
                       {report.overall_average >= 90 ? 'Excellent' : report.overall_average >= 80 ? 'Very Good' : 'Good Progress'}
                    </p>
                 </div>
                 
                 <div className="premium-card">
                    <h4 className="text-xs font-black text-[var(--edlink-blue-text)]/70 uppercase tracking-widest mb-6">Student Information</h4>
                    <div className="space-y-4">
                       <div>
                          <p className="text-[10px] text-[var(--edlink-blue-text)]/70 font-bold uppercase mb-0.5">Full Name</p>
                          <p className="text-sm font-bold text-white">{report.student.full_name}</p>
                       </div>
                       <div>
                          <p className="text-[10px] text-[var(--edlink-blue-text)]/70 font-bold uppercase mb-0.5">Institutional ID</p>
                 ...
                 <div id="report-card-canvas" className="premium-card bg-white p-8 md:p-12 text-black shadow-2xl border-none">
                    <div className="flex justify-between items-start mb-12 border-b-4 border-black pb-8">
                       <div>
                          ...
                          <p className="text-sm font-bold text-[var(--edlink-blue-text)]/70">OFFICIAL ACADEMIC TRANSCRIPT</p>
                          <p className="text-xs text-[var(--edlink-blue-text)]/70 mt-1 uppercase font-bold">{terms.find(t => t.id === selectedTerm)?.name}</p>
                       </div>
