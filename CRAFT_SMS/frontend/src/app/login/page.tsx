...
import { Eye, EyeOff, Lock, Mail, CloudLightning, ShieldCheck, BarChart3, CheckCircle2 } from "lucide-react";
            ...
            <div className="w-full max-w-xl rounded-[40px] bg-white p-8 shadow-[0_40px_120px_-48px_rgba(15,23,42,0.12)] sm:p-10">
              ...
              <form onSubmit={handleLogin} className="mt-8 space-y-6">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full rounded-3xl bg-[var(--brand-primary)] px-6 py-4 text-sm font-semibold text-white transition hover:bg-[#006342] disabled:cursor-not-allowed disabled:bg-slate-400"
                >
                  {isLoading ? "Signing in..." : "Sign in"}
                </button>
                {googleWarning && (
                  <div className="rounded-3xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-700">
                    Google sign-in is not available in this environment. Please use your school email and password.
                  </div>
                )}
              </form>
