    ...
    <>
      <div className="space-y-8">
        <header>
          ...
          <p className="text-[var(--edlink-blue-text)]/70">Monitor and control active/inactive state for all tenant schools.</p>
        </header>

        <div className="premium-card">
          {isLoading ? (
            <div className="flex justify-center py-16">
              <div className="w-8 h-8 border-4 border-[var(--edlink-green-brand)]/20 border-t-teal-500 rounded-full animate-spin" />
            </div>
          ) : schools.length === 0 ? (
            <div className="text-center py-16 text-[var(--edlink-blue-text)]/70">
                ...
                <tbody>
                  {schools.map(school => (
                    <tr key={school.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-4 pr-6 text-sm text-[var(--edlink-blue-text)]/70 font-mono">{school.subdomain}</td>
                      <td className="py-4 pr-6 text-sm text-[var(--edlink-blue-text)]/70">
                      ...
                      <td className="py-4 text-right">
                        <button
                          ...
                          className={`flex items-center gap-2 ml-auto px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                            school.is_active
                              ? 'bg-[var(--edlink-green-brand)]/10 text-emerald-400 hover:bg-[var(--edlink-green-brand)]/20'
