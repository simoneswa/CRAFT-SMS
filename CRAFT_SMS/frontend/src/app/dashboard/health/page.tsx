...
export default function SystemHealthPage() {
  ...
  const loadHealth = async () => {
    setIsLoading(true)
    try {
      const data = await fetchAPI('/health/status')
      setHealth(data)
    } catch (err) {
             ...
             <div className="md:col-span-2 space-y-8">
                <div className="premium-card relative overflow-hidden">
                   ...
                   <p className="text-xs text-gray-500 font-bold uppercase tracking-widest mt-4">
                      Total System Latency: <span className="text-white">{health?.total_latency_ms}ms</span>
                   </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                   <div className="premium-card">
                      ...
                      <p className="text-[10px] text-gray-500 font-bold uppercase mt-2">Latency: {health?.latency?.database_ms}ms</p>
                   </div>
                   <div className="premium-card">
                      <div className="flex items-center gap-3 mb-6">
