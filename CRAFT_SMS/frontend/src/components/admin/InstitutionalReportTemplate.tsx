  ...
  return (
    <div className="bg-white text-black p-12 min-h-[297mm] w-[210mm] mx-auto shadow-2xl relative overflow-hidden" id="report-content">
      ...
      <div className="flex justify-between items-start border-b-4 border-black pb-8 mb-10">
        <div className="flex items-center gap-6">
           ...
           <div>
              ...
              <p className="text-xs font-bold text-[var(--edlink-blue-text)]/70 uppercase tracking-widest mt-2">Institutional Report Service</p>
           </div>
        </div>
        <div className="text-right">
           ...
           <p className="text-[10px] text-[var(--edlink-blue-text)]/70 uppercase mt-1">ID: {crypto.randomUUID().slice(0,8).toUpperCase()}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-12 mb-12">
         <div>
            <h2 className="text-xs font-black text-[var(--edlink-blue-text)]/70 uppercase tracking-widest mb-4">Report Specification</h2>
         ...
         <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 text-right">
            <h2 className="text-xs font-black text-[var(--edlink-blue-text)]/70 uppercase tracking-widest mb-2">Operational Status</h2>
            <p className="text-xl font-black text-black">VERIFIED & AUDITED</p>
            <p className="text-[10px] text-[var(--edlink-green-brand)] font-bold uppercase mt-1 flex items-center justify-end gap-1">
      ...
      <div className="space-y-10">
         {type === 'FINANCIAL' ? (
           <>
             <div className="grid grid-cols-3 gap-6 mb-10">
                <div className="border-l-4 border-black pl-4">
                   <p className="text-[10px] font-bold text-[var(--edlink-blue-text)]/70 uppercase mb-1">Total Collections</p>
                   <p className="text-2xl font-black">${data.summary.total_collected.toLocaleString()}</p>
                </div>
                <div className="border-l-4 border-gray-300 pl-4">
                   <p className="text-[10px] font-bold text-[var(--edlink-blue-text)]/70 uppercase mb-1">Pending Receivables</p>
                   <p className="text-2xl font-black">${data.summary.total_pending.toLocaleString()}</p>
                </div>
                <div className="border-l-4 border-gray-300 pl-4">
                   <p className="text-[10px] font-bold text-[var(--edlink-blue-text)]/70 uppercase mb-1">Collection Efficiency</p>
                   ...
                   {data.records.map((record: any, i: number) => (
                      <tr key={i}>
                         <td className="py-4">
                            ...
                            <p className="text-[10px] text-[var(--edlink-blue-text)]/70">{record.profiles.custom_id}</p>
                         </td>
           ...
           <div className="text-center py-20 bg-gray-50 rounded-3xl">
              <p className="text-[var(--edlink-blue-text)]/70 italic">Attendance heatmap data rendering is being finalized...</p>
           </div>
         )}
      </div>
      ...
      <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end pt-8 border-t border-gray-100">
         <div>
            <p className="text-sm font-black italic">CRAFT <span className="text-[var(--edlink-blue-text)]/70">SMS</span></p>
            <p className="text-[9px] text-[var(--edlink-blue-text)]/70 font-bold uppercase tracking-widest mt-1">Enterprise Educational Operating System</p>
         </div>
         <div className="text-right">
            <p className="text-[10px] font-bold text-[var(--edlink-blue-text)]/70 uppercase">Page 1 of 1</p>
         </div>
      </div>
    </div>
  )
